...
def handshake(client):
    ...
    client.send(shake)
    log.debug('Hand shake successful')
        ...
        while self.SERVER_ACTIVE:
            try:
                ...
                if err == 'timed out':
                    ...
                    return
        log.debug('Shutting down WS server port: %s' % self.port)
        return
        ...
        try:
            while self.SERVER_ACTIVE:
                ...
                for clientCode in self.clients.keys():
                    ...
                    try:
                        if self.clients[clientCode]['alive']:
                            ...
                            log.debug('%s Client lost' % clientCode)
                            toRemove.append(clientCode)
                    except Exception, e:
                        log.error('Thread for _poll crashed')
                        self.clients[clientCode]['alive'] = False
                        toRemove.append(clientCode)
                        log.error(e)
                ...
                if len(self.clients) == 0:
                    log.debug('No Clients, shutting down poller')
                    self.POLLER_ACTIVE = False
                    return
                time.sleep(15)
        except Exception, e:
        ...
        try:
            ...
            for clientCode in self.clients.keys():
                ...
                try:
                    ...
                    log.error('error writing - %s' % data)
                    self._remove(clientCode)
        except Exception, e:
            log.error(format())
            log.error(e)
...
# Socket connection for the FRAM page (sends compile messages to page)
framPageSocket = WebSocketServer(6789)
framPageSocket.startServer()
# Sockets for the Log page
